...
/**
 * Representation of a robot, including its physical characteristics such as
 * size and color, sensors and actuators and the controller.
 * 
 * @author alc
 */
public class Robot extends MovableObject {
	...
	/**
	 * The color of the robot's body. This color can change during a simulation.
	 * COLOR.Black means that the robot may be invisible to other nearby robots
	 * (unless some sensor/actuator pair that does not rely on color is used).
	 * 
	...
	/**
	 * Get a sensor with a specific id. The id of sensors and actuators are
	 * usually specified in a configuration file.
	 * 
	 * @param id
	 *            the id of a sensor
	 * @return the sensor with the id specified. If no such sensor is found, a
	...
	public Sensor getSensorWithId(int id) {
		...
		if (!found) {
			throw new RuntimeException(
					"Cannot find sensor with id: " + id + ". (" + sensors.size() + " sensors available)");
		}
	...
	/**
	 * Get an actuator with a specific id. The id of sensors and actuators are
	 * usually specified in a configuration file.
	 * 
	 * @param id
	...
	public Actuator getActuatorWithId(int id) {
		...
		if (!found) {
			throw new RuntimeException(
					"Cannot find actuator with id: " + id + ". (" + actuators.size() + " actuators available)");
		}
		return actuator;
	}
	 ...
	 * 
	 * @param actuator
	 *            the new actuator to add to the robot.
	 */
	 ...
	 * @param teleported
	 *            the list of teleported objects
	 * 
	 */
	...
	/**
	 * Update all actuators including moving the robot (using differential drive
	 * kinematics) according to the speeds of its left and right wheels.
	 * 
	 * @param time
	 *            the number of the current simulation step.
	 * @param timeDelta
	 ...
	 * 
	 * @param controller
	 *            the new controller for the robot.
	 */
	...
	/**
	 * Process key presses: forward keyboard events to sensors, actuator and
	 * controllers so that they can respond to it (mostly for debugging purposes
	 * and for interactive experiments).
	 */
	@Override
	...
	/**
	 * Process key releases: forward keyboard events to sensors, actuator and
	 * controllers so that they can respond to it (mostly for debugging purposes
	 * and for interactive experiments).
	 */
	@Override
	...
	/**
	 * Process key input: forward keyboard events to sensors, actuator and
	 * controllers so that they can respond to it (mostly for debugging purposes
	 * and for interactive experiments).
	 */
	@Override
	...
	/**
	 * Set the color of a robot's body using RGB values between 0..1.0 for red,
	 * green and blue.
	 *
	 * @param red
	 *            the value for red.
	...
	/**
	 * Get the color of a robot's body as three doubles (one for red, green and
	 * blue, respectively)
	 * 
	 * @return robot's body in three doubles each between 0..1.0 for red, green
	...
	public static ArrayList<Robot> getRobots(Simulator simulator, Arguments arguments) {
		...
		if (arguments.getArgumentAsIntOrSetDefault("enableRandomizenumber", 0) == 1
				&& arguments.getArgumentIsDefined("randomizenumber")) {
		...
		if (arguments.getArgumentIsDefined("randomize")) {
			int extra = simulator.getRandom().nextInt(arguments.getArgumentAsInt("randomize") * 2)
					- arguments.getArgumentAsInt("randomize");
	...
	private static void addActuators(Simulator simulator, Robot robot, Arguments arguments) {
		...
		for (int i = 0; i < actuators.getNumberOfArguments(); i++) {
			...
			Actuator actuator = Actuator.getActuator(simulator, actuatorArgs.getArgumentAsString("classname"),
					actuatorArgs);
